...
check = vector()
for(i in 1:length(names(nolog_vcv))){
  check[i] = is.singular.matrix(as.matrix(nolog_vcv[[i]]))
}

# check negative autovalues
check = vector()
for(i in 1:length(names(nolog_vcv))){
  autovals = eigen(as.matrix(nolog_vcv[[i]]))$values
...
for(i in 1:length(temp)){
  ...
  if(test == TRUE){
    ...
    for(k in 1:length(n_ssp)){
      ssp = str_split_1(names(nolog_vcv)[[i]], "_")[3] 
      ...
      autonomy = Autonomy(as.matrix(nolog_vcv[[i]]), iterations = 10000)
      auto = autonomy[2]
      auto_ICV = autonomy[3]
      
      flexibility = Flexibility(as.matrix(nolog_vcv[[i]]), iterations = 10000)
      flex = flexibility[2]
      ...
      var_by_mean_geo = sqrt(sum(diag(as.matrix(nolog_vcv[[i]])))) / subsp_med_geom
      ...
      traço_matriz = mean(sum(diag(as.matrix(nolog_vcv[[i]]))))
      ...
      teste_media_autovalues_evolv = mean(eigen(nolog_vcv[[i]])$values)
      ...
      to_data = c(paste(names(nolog_vcv)[[i]], n_ssp[k], sep = "_"), var_by_mean_geo, var_by_skull_size, sqrt_trace, r2, size_geomean, size_linham, traço_matriz,
    ...
    var_by_mean_geo = sqrt(sum(diag(as.matrix(nolog_vcv[[i]])))) / med_geom
    var_by_skull_size = sqrt(sum(diag(as.matrix(nolog_vcv[[i]])))) / tamanho_cranio
    sqrt_trace = sqrt(sum(diag(as.matrix(nolog_vcv[[i]]))))
    options(warn=-1)
    r2 = CalcR2(as.matrix(nolog_vcv[[i]]))
    ...
    to_data = c(names(nolog_vcv)[[i]], var_by_mean_geo, var_by_skull_size, sqrt_trace, r2, size_geomean, size_linham, traço_matriz,
                icv, cv_medio_geral, cv_medio_males, cv_medio_females, cv_geomean_geral, cv_geomean_males, cv_geomean_females, respondability_normalized,
                flexibility, evolvability_normalized, teste_media_autovalues_evolv, cond_evolvability_normalized, autonomy, bias_abs, 
                bias_abs_ICV, cv_f, cv_e_norm, cv_c_norm, cv_r_norm, cv_a, N_males, N_females, N_total, mean_machos, mean_femeas)
...
write.xlsx(data.final, file = "~/Dropbox/Doc/Code/evowm/R/Novo_Output/Table_paranicho_Catarrhini_2.xlsx")
#dados = read.xlsx("~/Dropbox/Doc/Code/evowm/R/Novo_Output/Table_paranicho_Catarrhini_2.xlsx")
...
dados = readRDS(file = "~/Dropbox/Doc/Code/evowm/R/Novo_Output/p_Averages_PCS_Extant_Species.RData")
...
write.xlsx(data.final, file = "~/Dropbox/Doc/Code/evowm/R/Novo_Output/Table_paranicho_Platyrrhini_2.xlsx")
#dados = read.xlsx("~/Dropbox/Doc/Code/evowm/R/Novo_Output/Table_paranicho_Platyrrhini_2.xlsx")
