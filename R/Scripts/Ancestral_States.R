...
# save plot
png(file = "~/Dropbox/Doc/Output/filogenia.png", res = 100)
plot(cat.tree)
nodelabels(frame = "n", cex = 0.8, col = "blue")
dev.off()

# open dimorianyNA# open dimoris.nan# open dimorphism estimations
...
dimorp = machos[,2:40] - femeas[,2:40]
...
# clone V/CV matrices based on cat.tree
genuses = unique(matings$GENUS)
list_vcv = list()
for(i in 1:length(genuses)){
  numb_vcv = sum(str_count(dimorp$species_tree, genuses[i]))
  vcvs = lapply(seq_len(numb_vcv), function(X) vcv[[which(names(vcv) == genuses[i])]])
  list_vcv = append(list_vcv, vcvs)
}

names(list_vcv) = species_tree

# estimate V/CV matrices from ancestral states
all_cov_matrices = PhyloW(cat.tree, list_vcv)
...
#x = setNames(all_pc2$"141", names(msrs[,50:88]))
#x[x > 0]
#x[x < 0]
...
#write.table(checar, file = "~/Dropbox/Doc/Output/Ancestrals_Pcs_Dimorp_Norma.txt", row.names = FALSE, dec = ".", sep = '\t', quote = FALSE)
...
par(mfrow = c(2, 2))
plot(checar$dimorp_PC1, checar$Dimorp_Norms, xlab = "Correlação Vetor de Dimorfismo e PC1", ylab = "Norma do Vetor de Dimorfismo")
plot(checar$dimorp_PC2, checar$Dimorp_Norms, xlab = "Correlação Vetor de Dimorfismo e PC1", ylab = "Norma do Vetor de Dimorfismo")
plot(checar$dimorp_PC3, checar$Dimorp_Norms, xlab = "Correlação Vetor de Dimorfismo e PC1", ylab = "Norma do Vetor de Dimorfismo")
plot(checar$dimorp_PC4, checar$Dimorp_Norms, xlab = "Correlação Vetor de Dimorfismo e PC1", ylab = "Norma do Vetor de Dimorfismo")

corVector(checar$dimorp_PC1, checar$Dimorp_Norms)
corVector(checar$dimorp_PC2, checar$Dimorp_Norms)
corVector(checar$dimorp_PC3, checar$Dimorp_Norms)
corVector(checar$dimorp_PC4, checar$Dimorp_Norms)
