...
species_catarrhini = paste(matings$GENUS, matings$SPECIES, sep = "_")
vcv = vcv[ names(vcv) %in% species_catarrhini ]
...
vcv = vcv[ names(vcv) %in% species_platyrrhini ]
...
for(i in 1:length(names(vcv))){
  ...
  # insert in list of species i
  extant_averages_pcs_c$Averages[[48+i]] = list(gen_avg_m, gen_avg_f)
  extant_averages_pcs_c$ByTrait_Averages[[48+i]] = list(avg_m, avg_f)
  extant_averages_pcs_c$PCs[[48+i]] = list(pc_1, pc_2, pc_3, pc_4, pc_5, pc_6, pc_7, pc_8)
  extant_averages_pcs_c$Autovalues[[48+i]] = list(vals_1, vals_2, vals_3, vals_4, vals_5, vals_6)
  extant_averages_pcs_c$Diagonal[[48+i]] = v
}
...
saveRDS(extant_averages_pcs_c, "~/Dropbox/Doc/Code/evowm/R/Outputs/averages_PCS_autovalues_primates.RDS")
