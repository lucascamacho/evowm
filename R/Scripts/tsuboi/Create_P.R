# CREATE P MATRICES WITH SAME DIM AS G SAGUINUS
...
msrs_catarrhini = read.csv(file = "~/Dropbox/Doc/Data/primates_measures/medidas_catarrhini.csv", 
                           dec = ",", sep = ",")
...
#
msrs_platyrrhini = read.csv(file = "~/Dropbox/Doc/Data/primates_measures/medidas_platyrrhini.csv", 
                            dec = ".", sep = ",")

# Check platyrrhini msrs
index = which(apply(msrs_platyrrhini[, 23:61], 1, 
                    function(x) any(is.na(x) | (x == "" & !is.numeric(x)))))
...
for(i in 1:length(species)){
  ...
  # REMOVE TRAITS IN ORDER TO HAVE THE SAME DIM AS G SAGUINUS
  # REMOVE PTTSP, EAMZYGO E MAIS UM...
