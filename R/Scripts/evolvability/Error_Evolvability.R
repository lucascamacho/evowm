...
p1 = ggplot(res_all, aes(x = sigma, y = theta_obs)) +
       ...
       labs(
         ...
         y = "Deviation of Estimative of evolvability",
         title = "Different sizes of P matrix (number of traits)"
  )

p1

ggsave("~/Dropbox/Doc/Code/evowm/R/Scripts/evolvability/Theoretical_Error_Evolvability.png", plot = p1,
...
#B <- rnorm(p)
B <- medidas$ByTrait_Averages$Cercocebus_agilis$Machos - medidas$ByTrait_Averages$Cercocebus_agilis$Fêmeas
...
p1 = ggplot(res, aes(x = sigma, y = theta_obs, color = type)) +
  stat_summary(fun = median, geom = "line", linewidth = 1) +
  ...
  labs(
    x = "Magnitude of the error (σ)",
    y = "Deviation of Estimation of evolvability",
    title = "Erro sob P aleatória vs modular"
  )

p1

ggsave("~/Dropbox/Doc/Code/evowm/R/Scripts/evolvability/Modular_vs_Theoretical.png", plot = p1,
       width = 12,    # largura em inches
       height = 8,   # altura em inches
       dpi = 200)    # resolução
