...
# Haplorrhini
d = gowdis(dados[ ,4:13])
fit = metaMDS(d, k = 2, trymax = 1000, maxit = 1000)

png(
  filename = "~/Dropbox/Doc/Code/evowm/R/Outputs/Haplorrhini_StressPlot.png",
...
x = as.data.frame(scores(fit)[,1])
y = as.data.frame(scores(fit)[,2])

especies = paste(dados$GENUS, dados$SPECIES, sep = "_")
data = data.frame(x, y, especies, dados$PARVORDER, dados$SOCIAL_ORGANIZATION, dados$MATING_SYSTEM, 
                  dados$PROP_MALES_FEMALES, dados$DOMINANCE, dados$AGGRESSION,dados$ALL_MALE_GROUPS,
                  dados$FURTIVE_COPULATION, dados$INFANTICIDE, dados$MULTILEVEL_SOCIETY, dados$N_PAPERS, dados$GENUS)

saveRDS(data, "~/Dropbox/Doc/Code/evowm/R/Outputs/Haplorrhini_MDS_Matings.RDS")

# 1. Extract scores of NMDS
scores_fit = as.data.frame(scores(fit)) %>%
...
cores <- c("#1b9e77", "#7570b3")
names(cores) <- parvorders

# Plot
mds <- ggplot(scores_fit, aes(x = NMDS1, y = NMDS2, color = data$dados.PARVORDER)) +
  
  # Elipses com mesmas cores dos pontos
  stat_ellipse(data = scores_fit, aes(x = NMDS1, y = NMDS2, group = data$dados.PARVORDER, fill = data$dados.PARVORDER),
              geom = "polygon", alpha = 0.2, color = NA) +

  geom_point(
    data = centroids,
    aes(x = NMDS1, y = NMDS2, color = `data$dados.PARVORDER`),
...
ggsave("~/Dropbox/Doc/Code/evowm/R/Outputs/Haplorrhini_MDS_Matings.png", plot = mds,
