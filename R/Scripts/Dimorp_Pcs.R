...
for(i in 1:length(vcv)){
  ...
  # choose species
  sub_species = msrs[which(msrs$species == names(vcv)[i]), ]
...
dimorp_pcs = cbind(dimorp_pcs, names(vcv))
dimorp_pcs = dimorp_pcs[, c("names(vcv)", "dimorp_PC1", "dimorp_PC2", "dimorp_PC3", "dimorp_PC4", "cor_iso_pc1", "cor_iso_pc2", "cor_iso_pc3", "cor_iso_pc4", "Dimorp_Norms")]

write.table(dimorp_pcs, file = "~/Dropbox/Doc/Output/Pcs_Dimorp_Norma.txt", row.names = FALSE, 
            dec = ".", sep = '\t', quote = FALSE)
...
plot(dimorp_pcs$dimorp_PC1, dimorp_pcs$Dimorp_Norms, xlab = "Alinhamento Vetor de Dimorfismo e PC1", ylab = "Norma do Vetor de Dimorfismo")
corVector(dimorp_pcs$dimorp_PC1, dimorp_pcs$Dimorp_Norms)
